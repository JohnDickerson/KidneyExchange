...
public class UNOSPair {
	...
	protected UNOSPair(Set<UNOSDonor> donors, UNOSRecipient recipient, boolean isAltruist) {
		this.donors = donors;
		...
		if(this.isAltruist) {
			this.pairID = -donors.iterator().next().getDonorID();
		} else {
			this.pairID = recipient.getRecipientID();
		}
	}
